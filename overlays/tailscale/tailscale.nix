# Fork of the nixpkgs tailscale package pointed at github.com/gotlougit/tailscale
# (upstream tailscale + Cloudflare WARP/MASQUE support).
# Kept in sync with pkgs/by-name/ta/tailscale/package.nix at nixpkgs f13ff45a.
#
# The fork's go.mod requires go >= 1.26.6 while nixpkgs' default go is 1.26.5,
# so the overlay passes a buildGoModule bound to go_1_27 (see default.nix).
{
  lib,
  stdenv,
...
buildGoModule (finalAttrs: {
  pname = "tailscale";
  version = "1.103.0-warp-2026-08-19";
  ...
  src = fetchFromGitHub {
    ...
    hash = "sha256-g8DrjH0nrW3xxIWUDSh7/SH6RH6/93wOYrGjxZcmlLg=";
  };

  vendorHash = "sha256-dGhhUiWeJk3k08Xc4Sf1ubfY/4Eytjo+JHVEdHHiu7Q=";
  ...
  meta = {
    homepage = "https://github.com/gotlougit/tailscale";
    description = "Node agent for Tailscale, a mesh VPN built on WireGuard (gotlougit fork with WARP support)";
